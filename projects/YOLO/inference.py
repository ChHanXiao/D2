...
if __name__ == '__main__':
    cfg_path = 'yamls/yolo/yolov5n-lp.yaml'
    model_path = 'work_dirs/yolov5n-lp/model_0019999.pth'

    img_dir = 'work_dirs/img_lp'
    savepath = 'work_dirs/result_lp'
    device = "cuda:0"
    show_thres = 0.5
    input_size = (640,640)
    ...
    for img_path in files:
        ...
        for k,processed_results in enumerate(results):
            ...
            keypoints = processed_results['instances'].pred_keypoints.detach().cpu().numpy()

            H, W = processed_results['instances'].image_size
            img_name = os.path.basename(img_path)
            for c,(x1,y1,x2,y2),s,lmk in zip(cls,bbox,scores,keypoints):
                if s < show_thres:
                    continue
                x1 = str(max(0, int(x1)))
                y1 = str(max(0, int(y1)))
                ...
                for i, point in enumerate(lmk):
                    cv2.circle(img_info['img'], (int(point[0]), int(point[1])), 4, clors[i], -1)
